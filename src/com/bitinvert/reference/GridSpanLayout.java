 ...
 * top row.
 * 
 * @author bitinvert / d10n
 */

public class GridSpanLayout implements LayoutManager,java.io.Serializable {
	/**
	 * Generated serial
	 */
	...
	/**
	 * Number of columns for the grid. Immutable. Must be a non-negative
	 * integer.
	 */
	final int cols;
	...
	/**
	 * Table that keeps track of spanned and allocated areas. The first
	 * dimension represents rows. The second dimension represents columns. The
	 * third dimension holds two integers; the first integer represents the row
	 * span of the area, and the second integer represents the column span of
	 * the area. Spans must be non-negative integers. A span of 0 denotes that
	 * the area is being allocated by another area.
	 */
	int spans[][][];
	...
	/**
	 * Creates a GridSpanLayout with the specified number of rows and columns,
	 * and with the specified spacing between rows and columns. Each area in the
	...
	/**
	 * Sets the amount of rows and columns that an area in the layout grid
	 * spans.
	 * 
	 * @param row
	 *        row of the area
		...
		for(int i = row + 1;i < row + rowSpan;i++) {
			for(int j = col + 1;j < col + colSpan;j++) {
				...
				if(rowSpanned || colSpanned) {
					throw new IllegalArgumentException(
							"Attempted to span into allocated area");
				}
			}
		}
		// </Validate>
	 ...
	 * layout.
	 * <p>
	 * The preferred width is determined by the width of the area that prefers
	 * the most width, multiplied by the number of columns, plus the width of
	 * the gaps between the columns, plus the left and right insets of the
	 * container.
	 * <p>
	 ...
	 * <p>
	 * The minimum width is determined by the width of the area that requires
	 * the most width, multiplied by the number of columns, plus the width of
	 * the gaps between the columns, plus the left and right insets of the
	 * container.
	 * <p>
	...
	public void layoutContainer(Container parent) {
		synchronized(parent.getTreeLock()) {
			...
			/* wExtra and hExtra are used to center the layout in the parent
			 * container */
